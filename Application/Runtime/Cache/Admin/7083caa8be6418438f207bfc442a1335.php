	...
	<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					...
					<div class="vv">
						<li><a class="btn btn-warning" href="<?php echo U('tianjia');?>">添加角色</a></li>
						<li>
							<form action="<?php echo U('Role/roleSearch');?>" method="POST">
								<input type="text" name="search">
								<input class="btn btn-warning" type="submit" value="查询角色">
							</form>
						</li>
					...
					<table class="table table-striped table-hover table-responsive">
						...
						<tbody>
							<?php if(is_array($info)): foreach($info as $k=>$v): ?><tr>
									<td><input type="checkbox" name="choose[]" value="<?php echo ($v["id"]); ?>"><?php echo ($v["role_id"]); ?></td>
									<td><?php echo ($v["role_name"]); ?></td>
									<td><?php echo ($v["role_auth_ids"]); ?></td>
									<td><?php echo ($v["role_auth_ac"]); ?></td>
									<td>
										<a class="btn btn-info" href="<?php echo U('distribute',array('role_id'=>$v['role_id']));?>">分配权限</a> 
										<a class="btn btn-danger" href="<?php echo U('del',array('role_id'=>$v['role_id']));?>">删除</a> 
									</td>
