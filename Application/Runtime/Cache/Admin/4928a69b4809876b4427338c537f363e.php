	...
	<div class="row">
			<div class="col-md-12">
				<div class="panel panel-default">
					...
					<div class="vv">
						<li><a class="btn btn-warning" href="<?php echo U('tianjia');?>">添加权限</a></li>
						<li>
							<form action="<?php echo U('Auth/authSearch');?>" method="POST">
								<input type="text" name="search">
								<input class="btn btn-warning" type="submit" value="查询权限">
							</form>
						</li>
					...
					<table class="table table-striped table-hover table-responsive">
						...
						<tbody>
							<?php if(is_array($info)): foreach($info as $k=>$v): ?><tr>
									<td><input type="checkbox" name="choose[]" value="<?php echo ($v["id"]); ?>"><?php echo ($v["auth_id"]); ?></td>
									<td><?php echo ($v["auth_name"]); ?></td>
									<td><?php echo ($v["auth_pid"]); ?></td>
									<td><?php echo ($v["auth_c"]); ?></td>
									<td><?php echo ($v["auth_a"]); ?></td>
									<td>
										<a class="btn btn-danger" class="btn btn-danger" href="<?php echo U('del',array('auth_id'=>$v['auth_id']));?>">删除</a> 
										<a class="btn btn-info" href="<?php echo U('upd',array('auth_id'=>$v['auth_id']));?>">修改</a>
									</td>
